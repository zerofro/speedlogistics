$(function () {
    load(getProjectName()+"/tAddress/getPage?pageNum=1&condition=")

    $(".searchBtn").click(function (event) {
        event.preventDefault();
        load(getProjectName()+"/tAddress/getPage?pageNum=1&condition="+$(".condition").val());
    });
...
function setTbody(list) {
    ...
    $.each(list, function (i, item) {
        var content = "<tr>" +
            "<td>" + item.wxName + "</td>" +
            "<td>" + (item.ownerName==null?'未知':item.ownerName) + "</td>" +
            "<td>" + (item.addressCity==null?'未知':item.addressCity) + "</td>" +
            "<td>" + (item.address==null?'未知':item.address) + "</td>" +
            "<td>" + (item.phone==null?'未知':item.phone) + "</td>" +
            "<td>" + (item.type==0?'寄件':'收件') + "</td>" +
            "</tr>";
...
function setPageBtn(page, condition) {
    ...
    const url = getProjectName() + "/tAddress/getPage?pageNum=";
    //清除旧数字页
    $(".numPage").remove();
